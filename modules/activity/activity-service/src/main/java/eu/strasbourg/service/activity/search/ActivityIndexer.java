...
public class ActivityIndexer extends BaseIndexer<Activity> {
	...
	protected Document doGetDocument(Activity activity) throws Exception {
		...
		long[] assetCategoryIds = AssetVocabularyHelper
			.getFullHierarchyCategoriesIds(activity.getCategories());
	...
	protected void reindexEntries(long companyId) throws PortalException {
		final IndexableActionableDynamicQuery indexableActionableDynamicQuery = ActivityLocalServiceUtil
		...
		indexableActionableDynamicQuery.setPerformActionMethod(
			new ActionableDynamicQuery.PerformActionMethod<Activity>() {
				...
				public void performAction(Activity entry) {
					try {
						...
						_log.error("Unable to index activity entry "
							+ entry.getActivityId());
					}
				}
