...
@Component(immediate = true, property = {
		...
		"mvc.command.name=savePublicHoliday" }, service = MVCActionCommand.class)
public class SavePublicHolidayActionCommand implements MVCActionCommand {
	...
	public boolean processAction(ActionRequest request, ActionResponse response)
		...
		try {
			...
			if (!isValid) {
				...
				response.setRenderParameter("cmd", "savePublicHoliday");
				return false;
			}
			...
			if (publicHolidayId == 0) {
				publicHoliday = _publicHolidayLocalService
						.createPublicHoliday(sc);
			} else {
				publicHoliday = _publicHolidayLocalService
			...
			publicHoliday.setNameMap(name);

			Date date = ParamUtil.getDate(request, "date",
					new SimpleDateFormat("dd/MM/yyyy"));
			publicHoliday.setDate(date);

			boolean recurrent = ParamUtil.getBoolean(request, "recurrent");
	...
	@Reference(unbind = "-")
	protected void setPublicHolidayLocalService(
