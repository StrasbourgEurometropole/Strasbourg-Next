            ...
            <div>
                ...
                <#if pageListing?? >
                    <div class="pro-wrapper">
                        <a href="${strasbourg.homeURL()?keep_before_last("/")}${pageListing}" class="pro-btn pro-btn-white">Tout voir</a>
                    </div>
                </#if>
            </div>
                        ...
                        <div class="item pro-bloc-card-budget ${classFaisable}" data-linkall="a">
                            <#if entry.getImageURL()?has_content>
                                    <figure role="group" class="fit-cover">
                                        <img src="${entry.getImageURL()}${(entry.getImageURL()?contains('?'))?then('&','?')}imagePreview=1" loading="lazy" width="155" height="200" alt="Image budget participatif"/>
                                    </figure>
                            </#if>
                            <div class="pro-header-budget">
                                <#if imageURL?has_content >
                                    <figure role="group">
                                        <img src="${imageURL}${(imageURL?contains('?'))?then('&','?')}imagePreview=1" loading="lazy" width="40" height="40" alt="Image du budget participatif"/>
                                    </figure>
                                </#if>
