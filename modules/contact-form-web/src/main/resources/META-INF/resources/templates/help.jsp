	...
	<c:choose>
		<c:when test="${not param.mailSent}">
			...
			<form action="${contactURL}" method="post" class="seu-main-form" onsubmit="return checkForm();">
				...
				<liferay-ui:error key="phone-error" message="phone-error" targetNode=".seu-main-form" />
				<liferay-ui:error key="object-error" message="object-error" targetNode=".seu-main-form" />
				<liferay-ui:error key="invalid-mail-error" message="eu.invalid-mail-error" targetNode=".seu-main-form" />
				<liferay-ui:error key="invalid-phone-error" message="eu.invalid-phone-error" targetNode=".seu-main-form" />
		...
		<c:otherwise>
			<div class="container pro-max-900">
				<div class="pro-wrapper">
					<p><liferay-ui:message key="success3" /></p>
				</div>
			</div>
		</c:otherwise>
	</c:choose>
