...
public class StopTimeUtil {
	...
	public static List<StopTime> findByStopId(
		...
		return getPersistence().findByStopId(
			stop_id, start, end, orderByComparator, retrieveFromCache);
	}
	...
	private static ServiceTracker<StopTimePersistence, StopTimePersistence>
		_serviceTracker;

	static {
		Bundle bundle = FrameworkUtil.getBundle(StopTimePersistence.class);

		ServiceTracker<StopTimePersistence, StopTimePersistence>
			serviceTracker =
				new ServiceTracker<StopTimePersistence, StopTimePersistence>(
					bundle.getBundleContext(), StopTimePersistence.class, null);
