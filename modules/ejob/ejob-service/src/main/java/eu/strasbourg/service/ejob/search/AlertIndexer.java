...
public class AlertIndexer extends BaseIndexer<Alert> {

	public static final String CLASS_NAME = Alert.class.getName();
	...
	@Override
	protected void doDelete(Alert alert) throws Exception {
		deleteDocument(alert.getCompanyId(),
			alert.getAlertId());
	}
	 ...
	 */
	@Override
	protected Document doGetDocument(Alert alert)
		throws Exception {
		Document document = getBaseModelDocument(CLASS_NAME, alert);
		...
		long[] assetCategoryIds = AssetVocabularyHelper
			.getFullHierarchyCategoriesIds(alert.getCategories());
		...
		document.addNumber(Field.STATUS, alert.getStatus());
		return document;
	}
	...
	protected void doReindex(String className, long classPK) throws Exception {
		Alert entry = AlertLocalServiceUtil
			.getAlert(classPK);
		doReindex(entry);
	}
	...
	protected void reindexEntries(long companyId) throws PortalException {
		final IndexableActionableDynamicQuery indexableActionableDynamicQuery = AlertLocalServiceUtil
		...
		indexableActionableDynamicQuery.setPerformActionMethod(
			new ActionableDynamicQuery.PerformActionMethod<Alert>() {
				...
				@Override
				public void performAction(Alert entry) {
					try {
						Document document = getDocument(entry);

						indexableActionableDynamicQuery.addDocuments(document);
					} catch (PortalException pe) {
						_log.error("Unable to index alert entry "
							+ entry.getAlertId());
					}
				}
